...
public class BuySubCommand extends AbstractSubCommand {
    ...
    @Inject
    public BuySubCommand(MessageConfiguration messageConfiguration) {
        super();
    ...
    public void executeCommand(CommandSender commandSender, String... arguments) {
        if (commandSender instanceof Player player) {
            if (arguments.length == 1) {
                ...
                if (perk == null) {
                    player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.perk_not_found",
                ...
                switch (result) {
                    case ERROR ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.error"));
                    case ALREADY_BOUGHT_PERK ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.already_bought",
                                    new ReplaceOptions("<perk>", perk.getIdentifier())));
                    case NO_PRICE_SET ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.no_price_set",
                                    new ReplaceOptions("<perk>", perk.getIdentifier())));
                    case NOT_ENOUGH_FUNDS ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.not_enough_money",
                                    new ReplaceOptions("<perk>", perk.getIdentifier())));
                    case NO_ECONOMY_INTERFACE ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.not_set_up"));
                    default ->
                            player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.success",
                                    new ReplaceOptions("<perk>", perk.getIdentifier())));
                }
            } else {
                player.sendMessage(this.messageConfiguration.getMessage("command.perks.buy.syntax"));
            }
        } else {
